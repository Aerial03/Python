...
'''
예외처리 구문
try: 예외가 발생할 가능성이 있는 코드를 정의하는 곳
except A: 발생한 예외를 잡기위한 객체를 정의하고 처리하는 곳
except B: 발생한 예외를 잡기위한 객체를 정의하고 처리하는 곳
else: 예외가 발생되지 않을 때 실행되는 곳
finally: 예외발생 여부와 상관없이 실행되는 곳
'''
...
for g in generator:
    print(g)  # 제네레이터를 한번 실행시켜서 데이터가 모두 소진되면 다시 돌릴 수 없음
